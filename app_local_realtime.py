...
if __name__ == '__main__':
    ...
    if prkn_handle <= 0:
        ...
        time.sleep(10) # 起動にこれくらいかかるのでとりあえず待つ
    ...
    try:
        while(True):
            ...
            rect_left,rect_top = win32gui.ScreenToClient(prkn_handle, (rect_left,rect_top))
            rect_right,rect_bottom = win32gui.ScreenToClient(prkn_handle, (rect_right,rect_bottom))
            print(cap_left, cap_top, cap_right, cap_bottom)
            print(rect_left,rect_top,rect_right,rect_bottom)
            exit(0)

            # 指定した領域内をクリッピング
    #         ImageGrab.grab(bbox=(cap_left,cap_top,cap_right,cap_bottom)).save("tmp/" + datetime.now().strftime('%Y%m%d%H%M%S%f') + ".png")
